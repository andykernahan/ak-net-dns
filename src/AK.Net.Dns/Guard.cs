...
namespace AK.Net.Dns
{
    ...
    internal static class Guard
    {
        ...
        internal static void IsValidBufferArgs(byte[] buffer, int offset, int count)
        {
            ...
            InRange(offset >= 0 && offset < buffer.Length, "offset");
            InRange(count >= 0 && offset + count <= buffer.Length, "count");
        }

        internal static void IsIPv4(IPAddress addr, string paramName)
        {
            NotNull(addr, paramName);
        ...
        {
            InRange(value >= ushort.MinValue && value <= ushort.MaxValue, paramName);
        }

        internal static void IsUInt32(long value, string paramName)
        {
            InRange(value >= uint.MinValue && value <= uint.MaxValue, paramName);
        }
